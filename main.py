...
COLORS={'primary':'#1f77b4','secondary':'#ff7f0e','success':'#2ca02c','danger':'#d62728','purple':'#9467bd','brown':'#8c564b','pink':'#e377c2','gray':'#7f7f7f','olive':'#bcbd22','cyan':'#17becf','black':'#2c3e50','background':'#fafafa'}
...
with tab1:
    ...
    with col1:
        st.subheader("CLOSE")
        fig_close=go.Figure()
        fig_close.add_trace(go.Scatter(x=list(range(len(df))),y=df["Close"],mode='lines',name='Close',line=dict(color=COLORS['primary'],width=1.5),fill='tozeroy',fillcolor='rgba(31,119,180,0.1)'))
        fig_close.update_layout(template='plotly_white',title=dict(text=f'Precio de Cierre - {token}',x=0.5,xanchor='center'),xaxis_title='Periodo',yaxis_title='Precio (USD)',height=400,margin=dict(l=50,r=30,t=60,b=50),hovermode='x unified',showlegend=False)
        fig_close.update_xaxes(showgrid=True,gridwidth=1,gridcolor='rgba(128,128,128,0.2)')
        fig_close.update_yaxes(showgrid=True,gridwidth=1,gridcolor='rgba(128,128,128,0.2)')
    ...
    with col2:
        ...
        fig_log=go.Figure()
        fig_log.add_trace(go.Bar(x=list(range(len(log_close))),y=log_close,marker_color=colors_log,name='Log Return',opacity=0.7))
        fig_log.add_hline(y=0,line_dash="solid",line_color=COLORS['black'],line_width=1)
        fig_log.add_hline(y=log_close.mean(),line_dash="dash",line_color=COLORS['secondary'],line_width=1,annotation_text=f"Media: {log_close.mean():.4f}",annotation_position="right")
        fig_log.update_layout(template='plotly_white',title=dict(text='Retornos Logaritmicos',x=0.5,xanchor='center'),xaxis_title='Periodo',yaxis_title='Log Return',height=350,margin=dict(l=50,r=30,t=60,b=50),showlegend=False,bargap=0)
        ...
        fig_norm=go.Figure()
        fig_norm.add_trace(go.Scatter(x=list(range(len(log_close_viz))),y=log_close_viz,mode='lines',name='Normalizado',line=dict(color=COLORS['purple'],width=1.5)))
        fig_norm.update_layout(template='plotly_white',title=dict(text='Retornos Normalizados [0,1]',x=0.5,xanchor='center'),xaxis_title='Periodo',yaxis_title='Valor Normalizado',height=300,margin=dict(l=50,r=30,t=60,b=50),showlegend=False)
...
with tab3:
    ...
    fig_mic=go.Figure()
    fig_mic.add_trace(go.Bar(y=df_importance['Feature'],x=df_importance['Score'],orientation='h',marker=dict(color=df_importance['Score'],colorscale='Viridis',showscale=True,colorbar=dict(title=dict(text='MIC Score',side='right'))),text=df_importance['Score'].round(3),textposition='outside',textfont=dict(size=10)))
    fig_mic.update_layout(template='plotly_white',title=dict(text='Maximal Information Coefficient (MIC) - Feature Importance',x=0.5,xanchor='center',font=dict(size=16)),xaxis_title='MIC Score',yaxis_title='',height=500,margin=dict(l=150,r=80,t=80,b=50),xaxis=dict(range=[0,df_importance['Score'].max()*1.15]))
...
with tab4:
    ...
    fig_tt=go.Figure()
    fig_tt.add_trace(go.Scatter(x=y_train.index.tolist(),y=y_train.tolist(),name='Train',mode='lines',line=dict(color=COLORS['primary'],width=2),fill='tozeroy',fillcolor='rgba(31,119,180,0.15)'))
    test_x=[i+len(y_train) for i in y_test.index.tolist()]
    fig_tt.add_trace(go.Scatter(x=test_x,y=y_test.tolist(),name='Test',mode='lines',line=dict(color=COLORS['secondary'],width=2),fill='tozeroy',fillcolor='rgba(255,127,14,0.15)'))
    fig_tt.add_vline(x=len(y_train),line_dash="dash",line_color=COLORS['danger'],line_width=2,annotation_text="Train/Test Split",annotation_position="top")
    fig_tt.update_layout(template='plotly_white',title=dict(text='Division Temporal: Train vs Test',x=0.5,xanchor='center',font=dict(size=16)),xaxis_title='Indice Temporal',yaxis_title='Valor Normalizado',height=400,margin=dict(l=50,r=30,t=80,b=50),legend=dict(orientation='h',yanchor='bottom',y=1.02,xanchor='right',x=1,bgcolor='rgba(255,255,255,0.8)'),hovermode='x unified')
    st.plotly_chart(fig_tt,use_container_width=True)
    fold_colors_train=['#1f77b4','#2ca02c','#9467bd','#8c564b','#e377c2']
    fold_colors_val=['#ff7f0e','#d62728','#bcbd22','#17becf','#7f7f7f']
    fig_folds=make_subplots(rows=k,cols=1,shared_xaxes=True,vertical_spacing=0.03,subplot_titles=[f'Fold {i+1}' for i in range(k)])
    for i,(t_idx,v_idx) in enumerate(wfrw(y_train,k=k,fh_val=30).split(y_train)):
        fig_folds.add_trace(go.Scatter(x=y_train.index[t_idx].tolist(),y=y_train.iloc[t_idx].tolist(),mode='lines',name=f'Train Fold {i+1}',line=dict(color=fold_colors_train[i],width=1.5),showlegend=(i==0)),row=i+1,col=1)
        fig_folds.add_trace(go.Scatter(x=y_train.index[v_idx].tolist(),y=y_train.iloc[v_idx].tolist(),mode='lines',name=f'Val Fold {i+1}',line=dict(color=fold_colors_val[i],width=2.5),showlegend=(i==0)),row=i+1,col=1)
    fig_folds.update_layout(height=800,template='plotly_white',title=dict(text='Walk-Forward Cross-Validation Folds',x=0.5,xanchor='center',font=dict(size=16)),margin=dict(l=50,r=30,t=100,b=50),showlegend=True,legend=dict(orientation='h',yanchor='bottom',y=1.02,xanchor='right',x=1))
    for i in range(k):
        fig_folds.update_yaxes(title_text='Valor',row=i+1,col=1)
    fig_folds.update_xaxes(title_text='Indice Temporal',row=k,col=1)
...
with tab5:
    ...
    if st.button("Iniciar Entrenamiento de Modelos",type="primary",use_container_width=True):
        ...
        if meta_model is not None:
            ...
            fig_loss.add_trace(go.Scatter(x=epochs,y=results['train_losses'],mode='lines',name='Train Loss',line=dict(color=COLORS['primary'],width=2)))
            fig_loss.add_trace(go.Scatter(x=epochs,y=results['val_losses'],mode='lines',name='Validation Loss',line=dict(color=COLORS['secondary'],width=2)))
            best_epoch=results['best_epoch']
            best_val_loss=results['val_losses'][best_epoch-1] if best_epoch<=len(results['val_losses']) else min(results['val_losses'])
            fig_loss.add_trace(go.Scatter(x=[best_epoch],y=[best_val_loss],mode='markers',name=f'Best Epoch ({best_epoch})',marker=dict(color=COLORS['danger'],size=12,symbol='star')))
            fig_loss.update_layout(template='plotly_white',title=dict(text='Curvas de Entrenamiento - MSE Loss',x=0.5,xanchor='center',font=dict(size=16)),xaxis_title='Epoch',yaxis_title='MSE Loss',height=450,margin=dict(l=50,r=30,t=80,b=50),legend=dict(orientation='h',yanchor='bottom',y=1.02,xanchor='right',x=1),hovermode='x unified')
            ...
            colors_weights=[COLORS['primary'],COLORS['success'],COLORS['purple'],COLORS['secondary']][:len(weights_df)]
            fig_weights.add_trace(go.Bar(x=weights_df['Modelo'],y=weights_df['Peso_Promedio'],marker=dict(color=colors_weights,line=dict(color='rgba(0,0,0,0.3)',width=1)),text=weights_df['Peso_Promedio'].round(3),textposition='outside',textfont=dict(size=12,color=COLORS['black'])))
            fig_weights.update_layout(template='plotly_white',title=dict(text='Pesos Promedio Asignados por LSTM a Cada Modelo Base',x=0.5,xanchor='center',font=dict(size=16)),xaxis_title='Modelo',yaxis_title='Peso Promedio (Softmax)',height=400,margin=dict(l=50,r=30,t=80,b=50),yaxis=dict(range=[0,weights_df['Peso_Promedio'].max()*1.2]))
            ...
            fig_evo=go.Figure()
            colors_evo=[COLORS['primary'],COLORS['success'],COLORS['purple'],COLORS['secondary']]
            for i,col in enumerate(results['model_names']):
                fig_evo.add_trace(go.Scatter(x=weights_evolution['index'],y=weights_evolution[col],mode='lines',name=col,line=dict(color=colors_evo[i%len(colors_evo)],width=2)))
            fig_evo.update_layout(template='plotly_white',title=dict(text='Pesos Dinamicos (alpha_t) por Modelo a lo Largo del Tiempo',x=0.5,xanchor='center',font=dict(size=16)),xaxis_title='Indice Temporal',yaxis_title='Peso (Softmax)',height=450,margin=dict(l=50,r=30,t=80,b=50),legend=dict(orientation='h',yanchor='bottom',y=1.02,xanchor='right',x=1),hovermode='x unified')
            ...
            fig_pred=go.Figure()
            fig_pred.add_trace(go.Scatter(x=pred_df['Indice'],y=pred_df['Real'],mode='lines',name='Real',line=dict(color=COLORS['black'],width=3)))
            fig_pred.add_trace(go.Scatter(x=pred_df['Indice'],y=pred_df['Meta_LSTM'],mode='lines',name='Meta LSTM',line=dict(color=COLORS['danger'],width=2.5,dash='dash')))
            if 'LGB' in pred_df.columns:
                fig_pred.add_trace(go.Scatter(x=pred_df['Indice'],y=pred_df['LGB'],mode='lines',name='LightGBM',line=dict(color=COLORS['primary'],width=1.5),opacity=0.7))
            if 'CatBoost' in pred_df.columns:
                fig_pred.add_trace(go.Scatter(x=pred_df['Indice'],y=pred_df['CatBoost'],mode='lines',name='CatBoost',line=dict(color=COLORS['success'],width=1.5),opacity=0.7))
            if 'TimeXer' in pred_df.columns:
                fig_pred.add_trace(go.Scatter(x=pred_df['Indice'],y=pred_df['TimeXer'],mode='lines',name='TimeXer',line=dict(color=COLORS['purple'],width=1.5),opacity=0.7))
            if 'Moirai' in pred_df.columns:
                fig_pred.add_trace(go.Scatter(x=pred_df['Indice'],y=pred_df['Moirai'],mode='lines',name='Moirai-MoE',line=dict(color=COLORS['secondary'],width=1.5),opacity=0.7))
            fig_pred.update_layout(template='plotly_white',title=dict(text='Predicciones de Todos los Modelos vs Valor Real (Normalizado)',x=0.5,xanchor='center',font=dict(size=16)),xaxis_title='Indice Temporal',yaxis_title='Valor Normalizado',height=500,margin=dict(l=50,r=30,t=80,b=50),legend=dict(orientation='h',yanchor='bottom',y=1.02,xanchor='center',x=0.5),hovermode='x unified')
            ...
            fig_metricas=make_subplots(rows=2,cols=2,subplot_titles=['MSE por Modelo','RMSE por Modelo','MAE por Modelo','R2 por Modelo'],vertical_spacing=0.15,horizontal_spacing=0.1)
            ...
            fig_metricas.update_layout(template='plotly_white',title=dict(text='Comparacion de Metricas entre Modelos',x=0.5,xanchor='center',font=dict(size=18)),height=700,margin=dict(l=50,r=50,t=100,b=50))
            fig_metricas.update_xaxes(tickangle=45,row=1,col=1)
            fig_metricas.update_xaxes(tickangle=45,row=1,col=2)
            fig_metricas.update_xaxes(tickangle=45,row=2,col=1)
            fig_metricas.update_xaxes(tickangle=45,row=2,col=2)
